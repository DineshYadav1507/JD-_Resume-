...
const empty = { company: "", title: "", dates: "", bullets: [""] };
const projectEmpty = { name: "", description: "", technologies: [""] };
...
export default function Profile() {
  ...
  useEffect(() => {
    fetch("/api/profile").then(async (x) =>
      x.ok ? setP(await x.json()) : r.push("/login"),
    );
  }, []);

  const set = (k: string, v: any) => setP((x: any) => ({ ...x, [k]: v }));
  const add = (k: string, item: any) => set(k, [...(p[k] || []), item]);
  const remove = (k: string, i: number) => set(k, p[k].filter((_: any, n: number) => n !== i));
  const update = (k: string, i: number, field: string, v: any) =>
    set(k, p[k].map((x: any, n: number) => (n === i ? { ...x, [field]: v } : x)));
  ...
  async function save() {
    ...
    setSaved(x.ok ? "Saved successfully" : "Save failed");
  }
  ...
  return (
    <main className="shell">
      ...
      <section className="card">
        ...
        <div className="grid">
          {["headline", "phone", "location", "linkedin", "website"].map((k) => (
      ...
      <section className="card" style={{ marginTop: 18 }}>
        ...
        <div>
          {(p.skills || []).map((x: string, i: number) => (
            <span className="pill" key={i}>{x} <button onClick={() => remove("skills", i)}>×</button></span>
          ))}
        </div>
        <button className="btn alt" onClick={() => add("skills", "")}>+ Add skill</button>
      </section>
      ...
      <section className="card" style={{ marginTop: 18 }}>
        ...
        {(p.experiences || []).map((x: any, i: number) => (
          <div className="card" key={i} style={{ marginTop: 12 }}>
            ...
            <div className="field">
              <label>Dates</label>
              <input value={x.dates || ""} onChange={(e) => update("experiences", i, "dates", e.target.value)} />
            </div>
            <div className="field">
            ...
            <button className="btn alt" onClick={() => remove("experiences", i)}>Remove</button>
          </div>
        ))}
        <button className="btn" onClick={() => add("experiences", { ...empty, bullets: [""] })}>+ Add experience</button>
      </section>
        ...
        {(p.projects || []).map((x: any, i: number) => (
          <div className="card" key={i} style={{ marginTop: 12 }}>
            ...
            <div className="field">
              ...
              <input value={(x.technologies || []).join(", ")} onChange={(e) => update("projects", i, "technologies", e.target.value.split(",").map((s: string) => s.trim()).filter(Boolean))} />
            </div>
            <button className="btn alt" onClick={() => remove("projects", i)}>Remove</button>
          </div>
        ))}
        <button className="btn" onClick={() => add("projects", { ...projectEmpty, technologies: [""] })}>+ Add project</button>
      </section>
      ...
      <div style={{ position: "sticky", bottom: 16, marginTop: 18 }}>
        <button className="btn" onClick={save}>Save Master Resume</button>
